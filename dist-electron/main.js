...
var AUDIO_EXTS = new Set(["mp3", "wav", "ogg", "flac", "m4a", "opus", "webm", "aac"]);
...
var AudioManager = class {
  ...
  async unloadByKind(kind) {
    ...
    for (const mod of this.loaded)
      if (mod.kind === kind) await runPactl(["unload-module", mod.index]).catch(() => {});
      else keep.push(mod);
    this.loaded = keep;
  }
...
function registerAudioIpc(manager) {
  ...
  ipcMain.handle("audio:set-mic-passthrough", async (_event, enabled) =>
...
var shortcutHeld = false;
var overlayActive = false;
var activeOverlayDisplayId = null;
