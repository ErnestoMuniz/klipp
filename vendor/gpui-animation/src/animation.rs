    ...
    RenderOnce for AnimatedWrapper<E>
{
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        // Só anexa listeners ao elemento interno quando comportamento orientado
        // a evento foi configurado no wrapper (`on_hover`/`on_click` ou
        // `transition_on_*`). Com uso puramente declarativo (`transition_when*`)
        // nenhum listener é registrado — o que também evita um segundo
        // `on_hover` no elemento interno (o GPUI permite só um por elemento).
        if on_hover_cb.is_some() || self.transitions.contains_key(&Event::HOVER) {
        ...
        if on_click_cb.is_some() || self.transitions.contains_key(&Event::CLICK) {
